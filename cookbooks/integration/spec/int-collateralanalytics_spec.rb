...
describe 'integration::int-collateralanalytics' do
  ...
  CA_PROPS_FILE = '/opt/tomcat/conf/int-collateralanalytics.properties'
  ...
  before do
	...
	@chef_run =
	  ...
	  end.converge 'integration::int-collateralanalytics'
  end

  it 'should set logging parameters correctly' do
    expect(@chef_run).to create_file CA_PROPS_FILE
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.ca.logback.maxFileSize=1KB'
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.ca.logback.maxHistory=1999'
  end

  it 'should set amqp parameters correctly' do
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.amqp.host=rabbitmq'
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.amqp.port=211'
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.amqp.username=test'
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.amqp.password=spec'
    expect(@chef_run).to create_file_with_content CA_PROPS_FILE,'rt.int.amqp.virtual-host=vhost'
  end  
end
