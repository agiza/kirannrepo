#
# Cookbook Name:: altisource
# Definition:: search_provider
#

define :search_provider, :appnames => {}, :port => nil do
  "#{params[:name]}ip" = []
  appnames["#{params[:appnames]}"].split(" ").each do |app|
    search(:node, "recipes:*\\:\\:#{app} AND chef_environment:#{params[:environment]}".each do |worker|
      "#{params[:name]}ip" << worker["ipaddress"]
    end
  end
  if "#{params[:name]}ip".nil? || "#{params[:name]}ip".empty?
    Chef::Log.warn("No #{params[:name]} servers found in search.") && "#{params[:name]}ip" = "No servers found."
  else
    params[:name] = "#{params[:name]}ip".sort.first
    #params[:port] = params[:port]
  end
  node.default."#{params[:name]}" = params[:name]
  node.default."#{params[:port]}" = params[:port]
end
