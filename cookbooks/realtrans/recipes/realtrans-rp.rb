...
if node.attribute?('package_noinstall')
  ...
  if app_version.nil? || app_version.empty?
    ...
    if new_version.nil? || new_version.empty?
      ...
      app_version = new_version[:realtransrp_version]
      node.set['realtransrp_version'] = app_version
    end
  else
...
if node.attribute?('realdocproxy')
  ...
  rdochost = search(:node, "recipes:realdoc\\:\\:rabbitmqserver OR role:realdoc AND chef_environment:#{node.chef_environment}")
